...
import static com.github.atomishere.util.FUtil.SAVED_FLAGS_FILENAME;

public class SavedFlags extends FreedomService
{

    public SavedFlags(AtomFreedomMod plugin)
    {
        super(plugin);
    }
    ...
    public Map<String, Boolean> getSavedFlags()
    {
        ...
        File input = new File(AtomFreedomMod.plugin().getDataFolder(), FUtil.SAVED_FLAGS_FILENAME);
        if (input.exists())
        {
            try
            {
    ...
    public void setSavedFlag(String flag, boolean value)
    {
        ...
        try
        {
            final FileOutputStream fos = new FileOutputStream(new File(plugin.getDataFolder(), FUtil.SAVED_FLAGS_FILENAME));
